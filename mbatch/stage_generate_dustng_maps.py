...
from utils import FG_PATH_DICT, FSKYS
...
for dust_type, fit_params in fit_scaled_params.items():
    ...
    enmap.write_map(output_path(autils.get_dustfgtype_map_name(dust_type, fit_params['amplitude'], fit_params['tilt'], sim_id=1000)), rescaled_sky_car_map)
    
    for fsky in FSKYS:
        rescaled_sky_car_maps_mask = rescaled_sky_car_map * masks[fsky]
        enmap.write_map(output_path(autils.get_dustfgtype_map_name(dust_type, fit_params['amplitude'], fit_params['tilt'], sim_id=1000, fsky=fsky)), rescaled_sky_car_maps_mask)
