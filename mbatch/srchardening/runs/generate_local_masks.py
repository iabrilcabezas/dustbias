...
parser.add_argument("--exp", type=str, default='ACT')
...
assert args.skyfrac in FSKY_EXP[args.exp], f"skyfrac must be one of {FSKY_EXP[args.exp]}"
assert args.exp in ['ACT', 'SO'], f"exp must be one of {FSKY_EXP.keys()}"

mask = enmap.read_map(f'/rds/project/dirac_vol5/rds-dirac-dp002/ia404/fgs/masks/{MASK_EXP[args.exp]}_{args.skyfrac}_{WCS_EXP[args.exp]}_car_apo3deg.fits')
...
np.save(output_path(f"submap_coordinates_{args.exp}_{args.skyfrac}_{args.width_ra}_{args.width_dec}.npy"),np.array(mask_box))

for i in range(len(masks)):
    enmap.write_fits(output_path(f"mask_{args.exp}_{args.skyfrac}_{i}.fits"), masks[i], extra={})
