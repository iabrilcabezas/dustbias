...
def get_scaled_map_name(dust_type, sim_id, fsky, freq):
    tag = f'{dust_type}_{sim_id}_{fsky}_{freq:.0f}GHz'
    return f'map_car_scaled_{tag}.fits'

def get_recons_name(ipatch, args, tag, mf=False):
    ...
    tag += f'_{args.dust_type}_{args.dust_freq:.1f}GHz_{filter_label}_{args.skyfrac}_{ell_range}'

    return f'recons_{tag}_{ipatch}.txt'
