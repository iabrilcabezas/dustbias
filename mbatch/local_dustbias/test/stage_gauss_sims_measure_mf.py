...
comm,rank,my_tasks = mpi.distribute(args.nsims_mf)
s = stats.Stats(comm)
...
for n in range(npatches):
    ...
    for sim_id in my_tasks: 
        print(sim_id)
        np.random.seed(int(sim_id + args.set * 1000))
    ...
    if rank==0:
        ...
        # mf_data[f'mf_grad_set{args.set}'] = mf_alm

    mf_name = f'patch{n}_mf_set{args.set}_{args.nsims_mf}'
