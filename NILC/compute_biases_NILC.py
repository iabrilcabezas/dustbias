...
MASK_EXP = {'ACT': 'dr6v4_lensing_20240919_night_apodized_mask_',
            'SO': 'SOLAT'}
FREQ_EXP = {'ACT': [90, 150],
...
FG_TYPES = ['pysm_high'] # list(PYSM_STR.keys())
...
def compute_for_filters(args):
    ...
    filters_txt = np.loadtxt(f'obs_Cl_TT_EE_BB_{(args.ilc_type).upper()}.txt')
    filters = {}
    
    # tcls gets overwritten because filter stage has the correct cell (lensed) + noise
    filters['TT'] = filters_txt[1][:args.mlmax+1]
    filters['TE'] = np.zeros(5000) # !filters_txt[2]
    filters['EE'] = filters_txt[2][:args.mlmax+1]
    filters['BB'] = filters_txt[3][:args.mlmax+1]
    
    noise = {'TT': filters['TT'][:args.mlmax+1], 'EE': filters['EE'][:args.mlmax+1], 'BB': filters['BB'][:args.mlmax+1]} # Ignoring TE already
    
    return ucls, filters, noise
...
def get_Als_Res(args):
    ...
    ucls, tcls, _ = compute_for_filters(args)
...
def get_map_filter(oalms, Noise, args):
    ...
    if oalms.shape[0] == 3:
        ...
        almt = np.zeros_like(alme) # qe.filter_alms(oalms[0].copy(), filt_t, lmin = args.lmin, lmax = args.lmax)
...
def compute_biases(args, mask, fgkey, alm=False, fsky=None, wcs=None, ilc_type=None):
    if not alm:
        ...
        mapE = enmap.read_map(f'{args.maps_path}/{(args.ilc_type).upper()}/{fgkey}_{args.ilc_type}_CMB_E.fits')
        mapB = enmap.read_map(f'{args.maps_path}/{(args.ilc_type).upper()}/{fgkey}_{args.ilc_type}_CMB_B.fits')
        foreground_map = np.array([enmap.samewcs(np.zeros_like(mapE), mapE), mapE, mapB])
        mask_foreground_map = mask*foreground_map
        foreground_alms = cs.map2alm(mask_foreground_map, lmax=args.mlmax)
...
for fsky in FSKY_EXP[args.exp]:
    masks[fsky] = enmap.read_map(f'/rds/project/dirac_vol5/rds-dirac-dp002/ia404/fgs/NILC/dustbias_sims/masks/{MASK_EXP[args.exp]}{fsky}_cosine_apod_3_deg_dg_4.fits')
...
Noise = compute_for_filters(args)[2]
...
for fsky in args.fskys:
    for fg_type in args.fgtypes:
        ...
        raw4pt[f'{key}_{fsky}'] = compute_biases(args, masks[fsky], key, alm=True, fsky=fsky, wcs=wcs_tag, ilc_type=args.ilc_type)
